...
export const DashboardPage = () => {
  ...
  const [interviewStageFilter, setInterviewStageFilter] = useState<
    "all" | InterviewStage
  >("all");
  const [verdictFilter, setVerdictFilter] = useState<"all" | VerdictStatus>(
    "all",
  );
  ...
  useEffect(() => {
    const loadJobs = async () => {
      try {
        ...
        const records = await firebaseJobService.listByUser(currentUser.id);
        setJobs(records);
      } finally {
        setIsLoading(false);
      }
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((job) => {
      ...
      const matchesInterviewStage =
        ...
        job.interviewStage === interviewStageFilter;
      const matchesVerdict =
        verdictFilter === "all" || job.verdict === verdictFilter;
  ...
  const jobsByStatus = useMemo(() => {
    return filteredJobs.reduce<Record<JobStatus, JobApplication[]>>(
      (bucket, currentJob) => {
  ...
  const handleSaveJob = async (jobId: string, updates: JobUpdateInput) => {
    const updatedJob = await firebaseJobService.updateJob(jobId, updates);
  ...
  const handleSaveJobNotes = async (jobId: string, notes: string) => {
    const updatedJob = await firebaseJobService.updateJobNotes(jobId, notes);
  ...
  return (
    <section className="dashboard-page" aria-label="Career Pilot dashboard">
      ...
      <section
        className="dashboard-page__filters"
      ...
      >
          ...
          <select
            ...
            onChange={(event) =>
              setInterviewStageFilter(
                event.target.value as "all" | InterviewStage,
              )
            }
          >
        ...
        <label>
          ...
          <select
            value={verdictFilter}
            onChange={(event) =>
              setVerdictFilter(event.target.value as "all" | VerdictStatus)
            }
          >
