...
export const EditJobPage = () => {
  ...
  useEffect(() => {
    const loadJob = async () => {
      ...
      const record = await firebaseJobService.getJobById(jobId);
      setJob(record);
      setIsLoading(false);
    };
  ...
  const handleUpdate = async (input: JobCreateInput) => {
    ...
    await firebaseJobService.updateJobDetails(jobId, input);
  ...
  return (
    <section className="add-job-page" aria-label="Edit job page">
      <header className="add-job-page__header">
        ...
        <p>
          Modify role information, stage, and context without losing pipeline
          history.
        </p>
      </header>
